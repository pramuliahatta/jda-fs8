            ...
            <div class=" relative bg-white shadow-md dark:bg-gray-800 sm:rounded-lg ">
                <div class="flex flex-col items-center justify-between p-3 space-y-3 md:flex-row md:space-y-0 md:space-x-4">
                    <div class="w-full md:w-1/2">
                        <form id="productForm" method="GET" action="{{ route('products') }}" class="flex items-center">
                            ...
                            <div class="relative w-full">
                                ...
                                <input type="text" id="simple-search" name="search" value="{{ request('search') }}" class="block w-full p-2 pl-10 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-green-400 focus:border-green-400 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Cari">
                        ...
                        <div class="flex items-center w-full space-x-3 md:w-auto ">
                            ...
                            <div id="filterDropdown" class="z-10 hidden w-48 p-3 bg-white rounded-lg shadow dark:bg-gray-700">
                                ...
                                <ul class="space-y-2 text-sm" aria-labelledby="dropdownDefault">
                                    ...
                                    <li class="flex items-center">
                                        <input id="elektronik" name="categories[]" type="checkbox" value="Lainnya" {{ isset($categories) ? (in_array('Lainnya', $categories) ? 'checked' : '') : '' }} class="w-4 h-4 bg-gray-100 border-gray-300 rounded text-primary-600 focus:ring-primary-500 dark:focus:ring-primary-600 dark:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500" />
            ...
            <div class="mt-2 sm:mt-2 md:gap-6 lg:flex lg:items-start xl:gap-8">
                <div class="w-full mt-6 grid grid-cols-1 md:grid-cols-3 gap-4 sm:mt-8">
                    ...
                    <div class="space-y-6 overflow-hidden rounded-lg border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                        <a href="{{ route('products.show', $product['id']) }}" class="overflow-hidden rounded">
                            @if (!empty($product['product_photo']))
                            @foreach ($product['product_photo'] as $productPhoto)
                            @if ($loop->first)
                            <img class="mx-auto h-64 w-full rounded-lg object-cover dark:hidden" src="/{{ $productPhoto['photo'] }}" alt="imac image" />
                            @endif
                            @endforeach
                            @else
                            <img class="mx-auto h-64 w-full rounded-lg object-cover dark:hidden" src="/upload/product/noimage.jpg" alt="imac image" />
                        ...
                        <div class="flex flex-col gap-3">
                            {{-- product name --}}
                            <a href="{{ route('products.show', $product['id']) }}" class="text-4xl font-bold tracking-tight text-gray-900 dark:text-white capitalize truncate flex items-center">
                                <div class="{{ $categoryClass }} inline-flex items-center justify-center px-2 py-1 mr-2 text-xs font-bold leading-none rounded-lg">
                                    {{ $product['category'] }}
                                </div>
                            </a>

                            <a href="{{ route('products.show', $product['id']) }}" class="text-4xl font-bold tracking-tight text-gray-900 dark:text-white capitalize truncate">{{ $product['name'] }}</a>
                        ...
                        <div class="mt-6 flex items-center gap-2.5">
                            <a type="button" href="{{ route('products.detail', $product['id']) }}" class="inline-flex w-full items-center justify-center rounded-lg bg-green-500 px-5 py-2.5 text-sm font-medium  text-white hover:bg-green-600 focus:outline-none focus:ring-4 focus:ring-green-200 dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">
                                Lihat Produk
                            </a>
                        </div>
                    </div>
